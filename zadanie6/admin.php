...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!empty($_POST['delete'])) {
        ...
        setcookie('client_id', $member_id, time() + 12 * 30 * 24 * 60 * 60);
        ...
        $bio = $_POST['bio'];
        $policy = $_POST['policy'];
        $select = $_POST['superpowers'];
        $user = 'u47532';
        $pass = '5378974';
        $db = new PDO('mysql:host=localhost;dbname=u47532', $user, $pass, array(PDO::ATTR_PERSISTENT => true));

        $member_id = $_COOKIE['client_id'];
...
if (!empty($_SERVER['PHP_AUTH_USER']) && !empty($_SERVER['PHP_AUTH_PW'])) {
    ...
    $stmt = $db->prepare("SELECT pow.name AS name, count(*) AS amount FROM superclients JOIN powers3 pow ON power_id = pow.id GROUP BY power_id");
    $stmt->execute();
