...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (empty($errors) && !empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
        try {
            ...
            $superpowers = $db->prepare("SELECT distinct name from superclients join powers3 pow on power_id = pow.id where client_id = ?");
            $superpowers->execute(array($member));
            $result = $superpowers->fetchAll(PDO::FETCH_ASSOC);
            $values['superpowers'] = implode(',', $result);
    ...
    if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
        try {
            ...
            $powers->execute(array($client_id['id']));
            ...
            foreach ($superpowers as $value) {
                ...
                $powers = $db->prepare("INSERT INTO superclients SET power_id = ?, client_id = ? ");
                $powers->execute(array($power_id['id'], $client_id['id']));
            }
        } catch (PDOException $e) {
            print('Error : ' . $e->getMessage());
            exit();
        ...
        try {
            ...
            foreach ($superpowers as $value) {
                ...
                $powers = $db->prepare("INSERT INTO superclients SET power_id = ?, client_id = ? ");
                $powers->execute(array($power_id['id'], $client_id));
            }
        } catch (PDOException $e) {
            print('Error : ' . $e->getMessage());
            exit();
